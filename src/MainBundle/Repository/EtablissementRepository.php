...
class EtablissementRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function NbrVisitesParType($type){
        $query=$this->getEntityManager()->createQuery("SELECT count(v) FROM MainBundle:Visited v join v.favoris et WHERE et.type like :no ")
            ->setParameter('no', $type);
        return $query->getSingleScalarResult();
    }

    public function NbrLikesParType($type){
        $query=$this->getEntityManager()->createQuery("SELECT count(v) FROM MainBundle:Wishliste v join v.favoris et WHERE et.type like :no ")
            ->setParameter('no', $type);
        return $query->getSingleScalarResult();
    }
